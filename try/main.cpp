...
glm::vec2 displayGazeData(){
	gazeData->update();
	glm::vec2 EyePosition = gazeData->getPoint2dAvg();
	EyePosition.y = WINDOW_HEIGHT - EyePosition.y;
...
#pragma region //set up shaders
	...
	GLuint h_modulationID = glGetUniformLocation(H_quad_programID, "modulation");
	...
	GLuint v_modulationID = glGetUniformLocation(V_quad_programID, "modulation");
...
#pragma endregion
	...
	//objMesh[defaultCue]->modulation = true;
...
#pragma endregion
	...
	do{
		//objMesh[defaultCue]->modulation = true;
		keyEventHandle();
		modulation = true;//reset modulation
...
#pragma region //saccadic data analysis, decide whether or not shut down the modulation
		...
		glm::vec2 eyetempPos = displayGazeData();
		  ...
		  printf(" eye center position is: %f, %f \n", eyeXPos_current, eyeYPos_current);
		...
		//	{
		//		//objMesh[defaultCue]->modulation = false;// turn off modulation
		//		modulation = false;
		//	}
		//}
		if (!(gazeData ->isFix()))
...
#pragma region //cue的false color rendering
		...
		for(int i = 0; i<5; i++){
			if(objMesh[i]->attractor){
				...
				if(dist_squared < 90000 ){//one percetual: 200 pixels
					objMesh[i]->attractor = false;
					objMesh[i]->edge = false;
					//objMesh[i]->modulation = false;
				...
				if(countUnknownTime >= 300 && dist_squared > 250000){
					objMesh[i]->edge = true;
				}
				if(dist_squared <= 250000 && dist_squared > 90000){
					objMesh[i]->edge = false;
					//objMesh[defaultCue]->modulation = true;
					blurFactor -= 0.001;
					brightness += 0.001;
...
#pragma region			//水平方向的高斯模糊处理
		...
		/*for(int i = 0; i<5; i++){
			if(objMesh[i]->attractor){
				glUniform1i(h_modulationID, objMesh[i]->modulation);
			}
		}*/
		glUniform1i(h_modulationID, modulation);
...
#pragma region	//竖直方向的高斯模糊处理,并将最后处理过的texture显示到屏幕上
		...
		glUniform1i(v_modulationID, modulation);
		for(int i = 0; i<5; i++){
			if(objMesh[i]->attractor){
				glUniform1i(edgeCueID, objMesh[i]->edge);
				//glUniform1i(v_modulationID, objMesh[i]->modulation);
			}
		}
		quadDraw(quad_vertexbuffer);
		glUseProgram(0);
